...
public class CodeTools
{
  ...
  protected void makeInList(boolean quoteElements)
  {
    ...
    int startPos = editor.getSelectionStart(startline) - editor.getLineStartOffset(startline);

    int endline = editor.getSelectionEndLine();
    int endpos = editor.getSelectionEnd(endline) - editor.getLineStartOffset(endline);
    ...
    try
    {
      ...
      {
        endline--;
        endpos = editor.getSelectionEnd(endline) - editor.getLineStartOffset(endline);
      }
    }
    catch (Exception e)
    {
    ...
    for (int i=startline; i <= endline; i++)
    {
      ...
      if (i == startline && startPos > 1)
      {
        line = line.substring(startPos - 1).trim();
      }
      if (i == endline && endpos < line.length())
      {
