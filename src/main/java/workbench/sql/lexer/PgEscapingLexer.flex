...
keyword=(
"PASSING"|"PLAN"|"LIMIT"|"OFFSET"|"CLUSTER"|"DISCARD"|"DEALLOCATE"|"DO"|"(IMPORT{wsp}FOREIGN{wsp}SCHEMA)"|"VACUUM"|(FOREIGN{wsp}DATA{wsp}WRAPPER)|(SKIP{wsp}LOCKED)|(NO{wsp}KEY{wsp}UPDATE)|(KEY{wsp}SHARE)|(MATCH{wsp}FULL)|(MATCH{wsp}PARTIAL)|(MATCH{wsp}SIMPLE)|(DEFAULT{wsp}PRIVILEGES)|
(ALTER{wsp}SESSION)|
"TRANSFORM"|
(PRIMARY{wsp}KEY)|
(FOREIGN{wsp}KEY)|
...
(WITHIN{wsp}GROUP)|
(GROUPING{wsp}SETS)|
(NESTED{wsp}PATH)|(ON{wsp}ERROR)|(IS{wsp}DOCUMENT)|(IS{wsp}NOT{wsp}DOCUMENT)
(OWNED{wsp}BY)|
